...
namespace log4net.Util {
  ...
  public sealed class NativeError {
    ...
    /// </para>
    /// </remarks>
    NativeError(int number, string message) {
      m_number = number;
      m_message = message;
    }
    ...
    public static string GetErrorMessage(int messageId) {
      ...
      // The function should allocates a buffer large enough to hold the formatted message
      ...
      var argumentsPtr = new IntPtr();
      // Pointer to array of values to insert, not supported as it requires unsafe code
      ...
      if (messageId != 0) {
        ...
        if (messageSize > 0) // Remove trailing null-terminating characters (\r\n) from the message
          msgBuf = msgBuf.TrimEnd(new[] {'\r', '\n'});
        else // A message could not be located.
          msgBuf = null;
      } else
        msgBuf = null;
    ...
    public override string ToString() {
      return string.Format(CultureInfo.InvariantCulture, "0x{0:x8}", Number) +
