        ...
        if (m_cb != null && m_cb.Length > 0)
          if (m_lossy) {
            ...
            if (flushLossyBuffer)
              ...
              } else // No lossy evaluator, all buffered events are discarded
                m_cb.Clear();
          } else // Not lossy, send whole buffer
    ...
    protected override void Append(LoggingEvent loggingEvent) {
      ...
      if (m_cb == null || m_bufferSize <= 1) {
        ...
        if ((!m_lossy) ||
          ...
          if (m_eventMustBeFixed) // Derive class expects fixed events
        ...
        if (discardedLoggingEvent != null) // Buffer is full and has had to discard an event
          if (!m_lossy) // Not lossy, must send all events
            ...
            if (m_evaluator != null && m_evaluator.IsTriggeringEvent(loggingEvent))
              SendFromBuffer(discardedLoggingEvent, m_cb);
            else if (discardedLoggingEvent != null) // Just send the discarded event
              SendBuffer(new[] {discardedLoggingEvent});
          }
        else // Buffer is not yet full
