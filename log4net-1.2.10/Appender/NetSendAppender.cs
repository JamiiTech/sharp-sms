  ...
  public class NetSendAppender : AppenderSkeleton {
    ...
    protected override void Append(LoggingEvent loggingEvent) {
      ...
      using (m_securityContext.Impersonate(this)) {
        // Send the message
        int returnValue = NetMessageBufferSend(Server, Recipient, Sender, renderedLoggingEvent,
                                               renderedLoggingEvent.Length*Marshal.SystemDefaultCharSize);

        // Log the error if the message could not be sent
        if (returnValue != 0) // Lookup the native error
          nativeError = NativeError.GetError(returnValue);
      }

      if (nativeError != null) // Handle the error over to the ErrorHandler
        ErrorHandler.Error(nativeError + " (Params: Server=" + Server + ", Recipient=" + Recipient + ", Sender=" +
                           Sender + ")");
    }
