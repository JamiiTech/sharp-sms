  ...
  public class AdoNetAppender : BufferingAppenderSkeleton {
    ...
    void InitializeDatabaseConnection() {
      try {
        ...
        using (SecurityContext.Impersonate(this)) // Open the database connection
          m_dbConnection.Open();
      } catch (Exception e) {
        // Sadly, your connection string is bad.
        ErrorHandler.Error("Could not open database connection [" + ConnectionString + "]", e);
          ...
          try {
            foreach (AdoNetAppenderParameter param in m_parameters)
              try {
                ...
                ErrorHandler.Error(
                  "Could not add database command parameter [" + param.ParameterName + "]", e);
                throw;
              }
          } catch {
            try {
