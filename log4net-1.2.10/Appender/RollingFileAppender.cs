  ...
  public class RollingFileAppender : FileAppender {
    ...
    protected override void OpenFile(string fileName, bool append) {
      lock (this) {
        ...
        } else if (LogLog.IsErrorEnabled) // Internal check that the file is not being overwritten
    ...
    void RollOverIfDateBoundaryCrossing() {
      if (m_staticLogFileName && m_rollDate)
        if (FileExists(m_baseFileName)) {
          ...
          if (
            !(last.ToString(m_datePattern, DateTimeFormatInfo.InvariantInfo).Equals(
            ...
            RollOverTime(false);
            LogLog.Debug("RollingFileAppender: curSizeRollBackups after rollOver at [" +
    ...
    void InitializeFromOneFile(string baseFile, string curFileName) {
      if (! curFileName.StartsWith(baseFile)) // This is not a log file, so ignore
        return;
      if (curFileName.Equals(baseFile)) // Base log file is not an incremented logfile (.1 or .2, etc)
        return;

      int index = curFileName.LastIndexOf(".");
      if (-1 == index) // This is not an incremented logfile (.1 or .2)
        return;
      ...
      if (m_rollDate && !m_staticLogFileName)
        if (
          ! curFileName.StartsWith(baseFile +
                                   m_dateTime.Now.ToString(m_datePattern, DateTimeFormatInfo.InvariantInfo))) {
      ...
      try {
        ...
        if (SystemInfo.TryParse(curFileName.Substring(index + 1), out backup))
          if (backup > m_curSizeRollBackups) {
            ...
            } else if (-1 == m_maxSizeRollBackups) // Infinite backups, so go as high as the highest value
              m_curSizeRollBackups = backup;
            else // Backups limited to a finite number
              if (m_countDirection >= 0) // Go with the highest file when counting up
                m_curSizeRollBackups = backup;
              else // Clip to the limit when counting down
    ...
    RollPoint ComputeCheckPeriod(string datePattern) {
      ...
      for (var i = (int) RollPoint.TopOfMinute; i <= (int) RollPoint.TopOfMonth; i++) {
        ...
        string r1 = NextCheckDate(s_date1970, (RollPoint) i).ToString(datePattern,
                                                                      DateTimeFormatInfo.InvariantInfo);

        LogLog.Debug("RollingFileAppender: Type = [" + i + "], r0 = [" + r0 + "], r1 = [" + r1 + "]");

        // Check if the string representations are different
        if (r0 != null && r1 != null && !r0.Equals(r1)) // Found highest precision roll point
          return (RollPoint) i;
      }
    ...
    protected void RollOverTime(bool fileIsOpen) {
      if (m_staticLogFileName) {
        ...
        if (fileIsOpen) // close current file, and rename it to datedFilename
          CloseFile();
      ...
      if (fileIsOpen) // This will also close the file. This is OK since multiple close operations are safe.
    ...
    protected void RollFile(string fromFile, string toFile) {
      if (FileExists(fromFile)) {
        ...
        LogLog.Warn("RollingFileAppender: Cannot RollFile [" + fromFile + "] -> [" + toFile +
    ...
    protected void DeleteFile(string fileName) {
      if (FileExists(fileName)) {
        ...
        try {
          ...
          if (fileToDelete == fileName) // Unable to move or delete the file
            ErrorHandler.Error("Exception while deleting file [" + fileToDelete + "]", deleteEx,
                               ErrorCode.GenericFailure);
          else // Moved the file, but the delete failed. File is probably locked.
    ...
    #region Nested type: DefaultDateTime
